 ...
 */
export function sanitizeVersion(branch: string): string {
  const digits = branch.replace(/\D/g, '');
  if (!digits) {
    throw new Error(
      `Branch "${branch}" contains no digits and cannot be used as a version suffix. ` +
 ...
 */
export function buildNewBranchName(headBranch: string, targetBranch: string): string {
