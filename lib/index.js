...
import { printCompletionScript } from "./completions.js";
import { Executor } from "./executor.js";
import { fetchPrInfo, createPr } from "./github.js";
import * as git from "./git.js";
import { pickCommits } from "./picker.js";
import { buildNewBranchName } from "./utils.js";
...
async function runCherryPickMode(opts) {
    ...
    for (const b of branches)
        log.dim(`  ${b}`);
    if (push)
        log.dim("  (will push each branch after cherry-pick)");
...
async function runPrBackportMode(opts) {
    ...
    for (const { targetBranch, newBranch } of plans) {
        ...
        try {
            ...
            log.step(`Cherry-picking ${selectedCommits.length} commit${selectedCommits.length === 1 ? "" : "s"}`);
            for (const commit of selectedCommits) {
                log.dim(`    ${commit.shortSha}  ${commit.message}`);
                git.cherryPick(commit, executor);
            }
...
function printSummary(results) {
    ...
    if (failed.length > 0) {
        log.info("");
        log.warn(`${failed.length} of ${results.length} branch(es) failed.` +
