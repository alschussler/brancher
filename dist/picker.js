...
exports.pickCommits = pickCommits;
const KEY = {
    UP: '\x1B[A',
    DOWN: '\x1B[B',
    SPACE: ' ',
 ...
 */
async function pickCommits(commits) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const state = {
            ...
            selected: new Set(commits.map((_, i) => i)),
        };
        let renderedLineCount = 0;
        function render(redraw) {
            const lines = [];
            lines.push(`${c.bold}Select commits to cherry-pick:${c.reset}`);
            lines.push(`${c.dim}↑/↓ navigate  Space toggle  a select all  n deselect all  Enter confirm  Ctrl+C abort${c.reset}`);
            lines.push('');
